...
typedef struct {
    ...
    /**
        ...
        This method should be called before any write operation to `inode` and its file content.
    ...
    /**
        ...
        Different from block cache, this method can either read or write the
        inode. If `do_write` is true and the inode is valid, write the content
        of `inode` to disk. If `do_write` is false and the inode is invalid,
        read the content of `inode` from disk.
